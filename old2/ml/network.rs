...
impl Network {
    pub fn new(input_size: usize, error_function: ErrorFunction, layers: Vec<LayerConfig>) -> Self {
        ...
        for layer in &layers {
            ...
            total_size += size;
            params_offsets.push(total_size);
    ...
    pub fn weights(&self, layer: usize, neuron: usize) -> &[f32] {
        ...
        let params_offset = self.params_offsets[layer];
        let offset = params_offset + neuron * (prev_size + use_biases) + use_biases;
    ...
    pub fn bias(&self, layer: usize, neuron: usize) -> Option<f32> {
        ...
        let offset = self.params_offsets[layer] + neuron * (prev_size + use_biases);
...
fn test_weights_biases_layout() {
    ...
    assert_eq!(net.bias(0, 0).unwrap(), net.params[0]);
    assert_eq!(net.weights(0, 0), &net.params[1..3]);
    assert_eq!(net.bias(0, 1).unwrap(), net.params[3]);
    println!("net prams: {:?}", net.params);
    assert_eq!(net.weights(1, 0), &net.params[6..]);
